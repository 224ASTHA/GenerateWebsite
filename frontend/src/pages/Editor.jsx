...
import { Code2, MessageCircle, Monitor, Rocket, Send, X } from 'lucide-react'
...
function WebsiteEditor () {
  ...
  const handleUpdate = async() => {
    ...
    setPrompt("")
    console.log("📤 sending:", {prompt: currentPrompt, id})
    try{
      const result = await axios.post(`${serverUrl}/api/website/update`, {prompt: currentPrompt, id},  
    ...
    <div className='h-screen w-screen flex bg-black text-white overflow-hidden'>
            ...
            {messages.map((m,i) => (
              ...
              className={`max-w-[85%] ${
                ...
                className={`px-4 py-2.5 rounded-2xl text-sm leading-relaxed ${
                  m.role === "user"
                  ? "bg-white text-black" : "bg-white/5 border border-white/10 text-zinc-200"
                }`}>
                  {m.content}
                </div>
              </div>
      ...
      <div className='flex-1 flex flex-col'>
        <div className='h-14 flex justify-between items-center border-b border-white/10 bg-black/80'>
          ...
          <div className='flex gap-2'>
            {website.deployed ? "" : <button className='flex items-center gap-2 px-4 py-1.5 rounded-lg
            bg-gradient-to-r from-indigo-500 to-purple-500 text-sm font-semibold transition'>
              <Rocket size={14}  onClick={handleDeploy}/> Deploy
            </button>}
            <button className='p-2 lg:hidden' onClick={() => setShowChat(true)}><MessageCircle size={18}/></button>
            <button className='p-2' onClick={() => setShowCode(true)}><Code2 /></button>
            <button className='p-2' onClick={() => setShowFullPreview(true)}><Monitor /></button>
          </div>
        </div>

        <iframe ref={iframeRef} sandbox='allow-scripts allow-forms'
      ...
      <AnimatePresence>
        {showFullPreview && 
           <motion.div className='fixed inset-0 z-[9999] bg-black'>
                ...
                className='absolute top-4 right-4 p-2 bg-black/70 rounded-lg'><X /></button>
           </motion.div>
        }
      </AnimatePresence>
