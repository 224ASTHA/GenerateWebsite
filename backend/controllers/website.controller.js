...
const changes = async (req, res) => {
    try {
        ...
        if (user.credits < 25) {
            return res.status(400).json({ message: "you don't have enough credits to update a website" })
        }
        ...
        let parsed = null
        for (let i = 0; i < 2 && !parsed; i++) {
            raw = await generateResponse(updatePrompt, 15000)
            parsed = await extractJson(raw)
        }

        if (!parsed) {
            raw = await generateResponse(updatePrompt + "\n\nRETURN ONLY RAW JSON", 15000)
            parsed = await extractJson(raw)
        }
...
export { generateWebsite, getWebsiteById, changes, getAll, deploy, getBySlug }